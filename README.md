...
**An online reach management application for the marketing manager at a growing small business, turning fragmented digital-presence signals into evidence-linked priorities, human-owned work, and a measured answer about whether that work helped.**

[**Live Demo →**](https://integratorjeffj.github.io/ReachOps/) · [Architecture blueprint](docs/implementation/reachops-architecture-blueprint.md) · [Product boundaries](#product-boundaries) · [Source and AI disclosure](#source-and-ai-disclosure)

`TypeScript` `Next.js 15` `NestJS 11` `PostgreSQL` `Prisma` `Zod` `Turborepo` `pnpm`

The organising idea is that a reporting product earns trust by being explicit about what it does not know. Every number carries the record it came from. Every recommendation carries the rule that produced it. Where something cannot be measured, the interface says so and explains why, rather than substituting a figure that looks like an answer. No signup, no setup.

## See it in 60 seconds

1. **Open the [Command Center](https://integratorjeffj.github.io/ReachOps/) and click an evidence chip.** Every headline number opens into the provider, definition, reporting period, and data quality behind it. Sessions rose 10.1% while the AC repair booking rate fell from 6.10% to 3.92%, and both figures name their source.
2. **Go to [Opportunities](https://integratorjeffj.github.io/ReachOps/opportunities/) and open REC-001.** It is the only one of six carrying a suggested explanation, held at medium confidence and labelled a hypothesis. The other five state plainly that no explanation is supported, which is the product refusing to guess.
3. **Read "What this cannot tell you" on the [AI answers panel](https://integratorjeffj.github.io/ReachOps/search/).** A cost prompt was cited in three consecutive Perplexity checks and absent on the fourth. On a panel that small it says the honest thing: that is variance, not a trend.
...
371 tests pass without a database, plus PostgreSQL-gated integration specs: 131 in the web application, 215 in the database package, 19 in contracts, 5 in integrations, and the API identity suite. The suites live under [`apps/web/src`](apps/web/src), [`packages/database/test`](packages/database/test), [`packages/contracts/test`](packages/contracts/test), [`packages/integrations/test`](packages/integrations/test), and [`apps/api/src`](apps/api/src), and [`ci.yml`](.github/workflows/ci.yml) runs them on every push.
